...
13. **(Completed)** Add `backtest` command suite with signal-based strategy backtesting:
    - `cryptool backtest run` — runs a single backtest with selected signals and combination method.
    - `cryptool backtest combo` — exhaustively tests all signal subsets and ranks by Sharpe ratio.
    - Signals: RSI, MACD, Bollinger Bands, EMA Crossover.
    - Combination methods: voting, consensus, weighted.
    - Profitability gate: min edge, risk/reward ratio filter applied before each entry.
    - Metrics: total return (after fees, slippage, tax), Sharpe, Sortino, max drawdown, Calmar, win rate.
    - Optional `--save` flag persists results to `backtest_results` table (migration 0006).
