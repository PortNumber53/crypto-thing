...
- Exchange data fetch for Coinbase:
  - `cryptool exchange coinbase data fetch <start-date> <end-date> <granularity>`
...
- Coinbase client: Minimal Advanced Trade client with HMAC auth and `candles` endpoint.
  - Prefer JWT auth when `COINBASE_API_KEY_NAME` and `COINBASE_API_PRIVATE_KEY` are provided.
  - Implements request rate limiting (RPM) and retry with exponential backoff.
  - Candle fetch batching for large ranges with `limit` and sub-window iteration.
